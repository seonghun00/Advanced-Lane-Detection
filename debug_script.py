...
cal_images = np.array([plt.imread(img) for img in cal_filenames], dtype=object)

# Importing test images
test_filenames = glob.glob('test_images/*.jpg')
test_images = np.array([plt.imread(img) for img in test_filenames], dtype=object)
# Chessboard edges
nx = 9
ny = 6
...
source = np.float32([ # MASK
    [offset, img_y-offset], # bottom left
    [img_x-offset, img_y-offset], # bottom right
    [offset, offset], # top left
    [img_x-offset, offset]]) # top right
...
plt.subplot(2, 2, 1)
plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)) # [수정]
plt.title("Original Image")
plt.subplot(2, 2, 2)
plt.imshow(cv2.cvtColor(cv2.undistort(img, mtx, dist, None, mtx), cv2.COLOR_BGR2RGB)) # [수정]
...
def pers_transform(img, nx=9, ny=6):
    ...
    # [수정]
    dst = np.float32([
        [src[0][0] + offset[0], src[0][1] + offset[1]], # bottom left
        [src[0][0] + offset[0], 0],                     # top left
        [src[3][0] - offset[0], 0],                     # top right
        [src[3][0] - offset[0], src[3][1] - offset[1]]  # bottom right
    ])
...
def mask_image(image):
    ...
    cv2.fillPoly(mask, [vertices], [255,255,255]) # [수정] 대괄호 추가
...
def test_process(img):
    ...
    warped, M, Minv = pers_transform(masked)
...
for i in range(6) :
    ax[i,0].imshow(test_images[i].astype(np.uint8)) # [수정]
    thresholded_img, warped, Minv = test_process(test_images[i].astype(np.uint8)) # [수정]
...
def window_search(binary_warped):
    ...
    midpoint = int(histogram.shape[0]/2)  # [수정]
    ...
    window_height = int(binary_warped.shape[0]/nwindows)  # [수정]
    ...
    for window in range(nwindows):
        ...
        if len(good_left_inds) > minpix:
            leftx_current = int(np.mean(nonzerox[good_left_inds])) # [수정]
        if len(good_right_inds) > minpix:        
            rightx_current = int(np.mean(nonzerox[good_right_inds])) # [수정]
    ...
    right = np.asarray(list(zip(right_fitx, ploty)), np.int32)  # tuple -> list
    left = np.asarray(list(zip(left_fitx, ploty)), np.int32)  # tuple -> list
...
def margin_search(binary_warped):
    ...
    cv2.fillPoly(window_img, np.int32([left_line_pts]), (0,255,0))  # [수정] np.intc -> np.int32
    cv2.fillPoly(window_img, np.int32([right_line_pts]), (0,255,0))  # [수정] np.intc -> np.int32
    ...
    # Draw polyline on image
    right = np.asarray(list(zip(right_fitx, ploty)), np.int32) # [수정] tuple -> list
    left = np.asarray(list(zip(left_fitx, ploty)), np.int32) # [수정] tuple -> list
    cv2.polylines(out_img, [right], False, (1,1,0), thickness=5)
    cv2.polylines(out_img, [left], False, (1,1,0), thickness=5)
...
for i in range(6) :
    thresholded_img, warped, Minv = test_process(test_images[i].astype(np.uint8)) # [수정] np.uint8 추가
    ax[i,0].imshow(thresholded_img, cmap='gray')
    left, right, polynom_img = window_search(thresholded_img.astype(np.uint8)) # [수정] np.uint8 추가
    ax[i,1].imshow(polynom_img.astype(np.uint8)) # [수정] np.uint8 추가
for axes in ax.flatten() :
    axes.axis('off')
...
class Line():
    def __init__(self, maxSamples=4):
        ...
        self.current_fit = np.array([0.0, 0.0, 0.0], dtype=np.float32) # [수정] dtype 추가
    ...
    def update_lane(self, ally, allx):
        ...
        self.best_fit = np.mean(list(self.recent_xfitted), axis=0) # [수정] list 추가
...
def validate_lane_update(img, left_lane_inds, right_lane_inds):
    ...
    print(f"DEBUG: left_len={len(left_line_allx)}, right_len={len(right_line_allx)}")
    if len(left_line_allx) <= 1800 or len(right_line_allx) <= 1800:
        print("DEBUG: Rejected by length")
    ...
    print(f"DEBUG: left_x_mean={left_x_mean}, right_x_mean={right_x_mean}")
    if left_x_mean > 740 or right_x_mean < 740:
        print("DEBUG: Rejected by x_mean")
        left_line.detected = False
        right_line.detected = False
        return
    
    # Discard the detections if the lane width is too large or too small
    print(f"DEBUG: lane_width={lane_width}")
    if  lane_width < 300 or lane_width > 800:
        print("DEBUG: Rejected by width")
        left_line.detected = False
        right_line.detected = False
        return 
...
def draw_lane(undist, img, Minv):
    ...
    if left_fit is not None and right_fit is not None:
        ...
        # Draw the lane onto the warped blank image
        cv2.fillPoly(color_warp, np.int32([pts]), (64, 224, 208)) # [수정] np.int_ -> np.int32
